...
public class ObjectLineMapper implements LineMapper
{
    ...
    public Object apply(List<String> rawValues)
    {
        ...
        for (Method fieldSetter : fieldSetterToMapper.keySet()) {
            Object fieldValue =
                this.fieldSetterToMapper.get(fieldSetter).apply(values.subList(columnOffset, values.size()));
    ...
    private void invokeSetter(Method setter, Object mappedObject, Object fieldValue)
    {
        try {
            ...
            throw new MapperException("A problem occured when invoking setter " + setter.getName() + " for class "
                + mappedObject.getClass().getName() + " with value " + fieldValue, e.getCause());
        } catch (IllegalAccessException e) {
            throw new MapperException(
                "Unaccessible method while mapping value : " + setter.getName() + ". Is it a public method?",
                e.getCause());
        }
    }
    ...
    private Object getInstance()
    {
        ...
        try {
            ...
            throw new MapperException("A problem occured when invoking default constructor "
                + this.constructor.getName() + " for class " + this.constructor.getDeclaringClass().getName(),
                e.getCause());
        } catch (IllegalAccessException e) {
            throw new MapperException(
                "Unaccessible constructor : " + this.constructor.getName() + ". Is it a public method?", e.getCause());
        }
    ...
    public static class ObjectLineMapperBuilder
    {
        ...
        private Constructor<?> getDefaultConstructor(Class<?> targetClass)
        {
            ...
            try {
                ...
                throw new MapperException("Could not obtain a default constructor for class " + targetClass.getName(),
                    e.getCause());
            }
        ...
        private Method findSetter(Field field)
        {
            ...
            try {
                ...
                throw new MapperException("Could not find a setter method " + setterName + "(" + argumentType.getName()
                    + " arg) for class " + field.getDeclaringClass().getName(), e.getCause());
            }
