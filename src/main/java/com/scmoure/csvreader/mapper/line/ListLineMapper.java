...
public class ListLineMapper implements LineMapper {
	...
	public List<?> apply(List<String> rawValues) {
		...
		return columnIndexes.stream()
				.map(i -> values.subList(i, values.size()))
				.map(elementMapper)
	...
	public static class ListLineMapperBuilder {
		...
		public ListLineMapperBuilder(Integer startingColumn, Integer endingColumn, Integer cycle,
				Class<?> targetType) {
		...
		public ListLineMapperBuilder withColumnFilter(Predicate<String> columnFilter) {
			this.columnIndexes = this.columnIndexes.stream()
					.filter(i -> columnFilter.test(this.columns.get(i)))
					.collect(Collectors.toList());
			return this;
		}
